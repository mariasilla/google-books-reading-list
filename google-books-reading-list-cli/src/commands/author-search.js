...
import { queryAuthorField, parseData } from "../utils/api-utils";
...
export function authorSearch() {
  ...
  (async () => {
    ...
    const { data } = await queryAuthorField(userQuery.value);
    const { status } = await queryAuthorField(userQuery.value);
    const { statusText } = await queryAuthorField(userQuery.value);
